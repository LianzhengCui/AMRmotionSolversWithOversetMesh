...
#include "dynamicMotionSolversRefineBalancedFvMesh.H"
...
namespace Foam
{
    defineTypeNameAndDebug(dynamicMotionSolversRefineBalancedFvMesh, 0);
    addToRunTimeSelectionTable(dynamicFvMesh, dynamicMotionSolversRefineBalancedFvMesh, IOobject);
	addToRunTimeSelectionTable(dynamicFvMesh, dynamicMotionSolversRefineBalancedFvMesh, doInit);
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //
Foam::label Foam::dynamicMotionSolversRefineBalancedFvMesh::topParentID(label p)
{
...
Foam::dynamicMotionSolversRefineBalancedFvMesh::dynamicMotionSolversRefineBalancedFvMesh
(
    const IOobject& io,
	const bool doInit
)
:
    dynamicMotionSolversRefineFvMesh(io, doInit),
    rebalance_(false)
{}

bool Foam::dynamicMotionSolversRefineBalancedFvMesh::init(const bool doInit)
{
    if (doInit)
    {
        dynamicMotionSolversRefineFvMesh::init(doInit);
    }
...
bool Foam::dynamicMotionSolversRefineBalancedFvMesh::update()
{

    //Part 1 - Call normal update from dynamicMotionSolversRefineFvMesh
    bool hasChanged = dynamicMotionSolversRefineFvMesh::update();
	...
	word cellSetName =
        dyMDict.getOrDefault<word>("cellSet", "none");
	...
	//return true if the mesh was refined and moved
    return hasChanged;
}

bool Foam::dynamicMotionSolversRefineBalancedFvMesh::loadBalancing()
{
    ...
    // Part 2 - Load Balancing
    {
        ...
        if ( Pstream::parRun() )
        {
            ...
            if( maxImbalance > allowableImbalance && enableBalancing)
            {
                ...
                autoPtr<mapDistributePolyMesh> balancingMap =
                    distributor.distribute(finalDecomp);

                Info<< "Distribute the map ..." << endl;
                meshCutter_->distribute(balancingMap);							
...
void Foam::dynamicMotionSolversRefineBalancedFvMesh::mapFields(const mapPolyMesh& mpm)
{
    dynamicFvMesh::mapFields(mpm);
}

bool Foam::dynamicMotionSolversRefineBalancedFvMesh::writeObject
(
    IOstreamOption streamOpt,
    const bool valid
) const
{
	bool writeOk = dynamicMotionSolversRefineFvMesh::writeObject(streamOpt, valid);
