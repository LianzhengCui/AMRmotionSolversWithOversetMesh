...
#include "rigidBodySolidMeshMotion.H"
...
namespace Foam
{
    defineTypeNameAndDebug(rigidBodySolidMeshMotion, 0);

    addToRunTimeSelectionTable
    (
        motionSolver,
        rigidBodySolidMeshMotion,
        dictionary
    );
}
...
Foam::rigidBodySolidMeshMotion::bodyMesh::bodyMesh
(
    const polyMesh& mesh,
    const word& name,
    const label bodyID,
...
Foam::rigidBodySolidMeshMotion::rigidBodySolidMeshMotion
(
    const polyMesh& mesh,
...
Foam::rigidBodySolidMeshMotion::curPoints() const
{
    tmp<pointField> newPoints(mesh().points() + pointDisplacement_.primitiveField());
    ...
    else
    {
        ...
        UIndirectList<point>(transformedPts, refinedPointIDs()) =
            pointField(newPoints.ref(), refinedPointIDs());
        return ttransformedPts;
    }
}

void Foam::rigidBodySolidMeshMotion::calcWeight(const polyMesh& curMesh) 
{
    forAll(bodyMeshes_, bi)
    {
		
		const pointMesh& pMesh = pointMesh::New(curMesh);
        
        pointPatchDist pDist(pMesh, bodyMeshes_[bi].patchSet_, curMesh.points());
...
void Foam::rigidBodySolidMeshMotion::updateBodyMesh(const polyMesh& mesh)
{
...
void Foam::rigidBodySolidMeshMotion::solve()
{
    const Time& t = mesh().time();
	
   // update body mesh and weight if the points number changed
    ...
    else
    {
        ...
        forAll(bodyIDs, bi)
        {
         ...
         pointDisplacement_.primitiveFieldRef() =
             model_.transformPoints(bodyIDs, weights, mesh().points()) - mesh().points();
    }
...
const Foam::labelList& Foam::rigidBodySolidMeshMotion::refinedPointIDs() const
{
    if(mesh().topoChanging())
	{	
       const cellSet& updateSet
	             = mesh().lookupObject<cellSet>("movingSet");
				 
		labelList newCellIDs;

        newCellIDs = updateSet.toc();
		
		boolList movePts(mesh().nPoints(), false);

        forAll(newCellIDs, i)
        {
            label celli = newCellIDs[i];
        ...
        updatePointIDs_.transfer(ptIDs);
	}
		return updatePointIDs_;
}

bool Foam::rigidBodySolidMeshMotion::writeObject
(
...
bool Foam::rigidBodySolidMeshMotion::read()
{
