...
namespace Foam
{
    defineTypeNameAndDebug(dynamicMotionSolversRefineFvMesh, 0);
    addToRunTimeSelectionTable
	(
	   dynamicFvMesh, 
	   dynamicMotionSolversRefineFvMesh, 
	   IOobject
	);
	addToRunTimeSelectionTable
	(
...
void Foam::dynamicMotionSolversRefineFvMesh::calculateProtectedCells
(
...
void Foam::dynamicMotionSolversRefineFvMesh::readDict()
{
...
void Foam::dynamicMotionSolversRefineFvMesh::mapFields(const mapPolyMesh& mpm)
{
...
Foam::dynamicMotionSolversRefineFvMesh::refine
(
...
)
{
	...
	if (!movingSet.empty())
    {
        ...
        forAll(cells(), celli)
        {
            // const label myOldCelli = map().cellMap()[celli];
			// cellIndexBeRefine[celli] = map().cellMap()[celli];
			
            if (movingSet.found(map().cellMap()[celli]))
            {
                newMyset.insert(celli);
            }
        }
        movingSet.transfer(newMyset);
    }
	movingSetPtr->store();
...
Foam::autoPtr<Foam::mapPolyMesh>
Foam::dynamicMotionSolversRefineFvMesh::unrefine
(
...
Foam::scalarField
Foam::dynamicMotionSolversRefineFvMesh::maxPointField(const scalarField& pFld) const
{
...
Foam::scalarField
Foam::dynamicMotionSolversRefineFvMesh::maxCellField(const volScalarField& vFld) const
{
...
Foam::scalarField
Foam::dynamicMotionSolversRefineFvMesh::cellToPoint(const scalarField& vFld) const
{
...
Foam::scalarField Foam::dynamicMotionSolversRefineFvMesh::error
(
...
void Foam::dynamicMotionSolversRefineFvMesh::selectRefineCandidates
(
...
Foam::labelList Foam::dynamicMotionSolversRefineFvMesh::selectRefineCells
(
...
void Foam::dynamicMotionSolversRefineFvMesh::extendMarkedCells
(
    bitSet& markedCell
) const
{
...
void Foam::dynamicMotionSolversRefineFvMesh::checkEightAnchorPoints
(
...
Foam::dynamicMotionSolversRefineFvMesh::dynamicMotionSolversRefineFvMesh
(
    const IOobject& io,
    const bool doInit
)
    ...
    )				
{
	...
	// initialise moving cellSet
	
	word cellSetName =
        dict.getOrDefault<word>("cellSet", "none");
		
	Info << "cellSetName is " << cellSetName << nl << endl;
	
	if (cellSetName != "none")
	{
	  cellSet originalSet(*this, cellSetName);
	  cellSet& movingSet = *movingSetPtr;
	  movingSet.transfer(originalSet);
	  movingSetPtr->store();
	  Info << "size of movingSet in constructors is " << movingSet.size() << nl << endl;
	}
	indexMapPtr->store();
}


bool Foam::dynamicMotionSolversRefineFvMesh::init(const bool doInit)
{
    if (doInit)
    {
...
bool Foam::dynamicMotionSolversRefineFvMesh::update()
{
	...
	const word dynamicFvMeshName = refineDict.get<word>("dynamicFvMesh");
	bool overset = (dynamicFvMeshName == "dynamicOversetRefineBalancedFvMesh");

    bool hasChanged = false;
	// bool hasRefined = false;
	// bool hasUnrefined = false;
    ...
    if (time().timeIndex() > 0 && time().timeIndex() % refineInterval == 0)
    {
        ...
        if (globalData().nTotalCells() < maxCells)
        {
            ...
            if (nCellsToRefine > 0)
            {
                ...
                // not been refined due to constraints.
                {
                    ...
                    indexCellMap.resize(cellMap.size());
					...
					//update map for overset mesh if it is used in a derived class
				   // if(overset)
				   // {					 
                       // labelIOList cellIndexMap
                      // (
                        // IOobject
                        // (
                         // "cellIndexMap",
...

        {
            ...
            {
                // Refine/update mesh
                autoPtr<mapPolyMesh> unrefineMap = unrefine(elemsToUnrefine);
				
				// OutPut the final map for overset mesh if exist 
				if(overset)
				{
					
					const labelIOList& cellRefineMap = 
					                 this->lookupObject<labelIOList>("cellIndexMap");
									 					
					
					forAll(unrefineMap().cellMap(), celli)
					{
						indexCellMap[celli] = unrefineMap().cellMap()[celli];
						if(hasChanged)
						{
						indexCellMap[celli] = cellRefineMap[unrefineMap().cellMap()[celli]];
						}
					}
	...
	bool hasMoved = dynamicMotionSolverListFvMesh::update();

    return (hasChanged && hasMoved);
}

bool Foam::dynamicMotionSolversRefineFvMesh::writeObject
(
