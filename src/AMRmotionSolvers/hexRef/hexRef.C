...
// void Foam::hexRef::distribute(const mapDistributePolyMesh& map, volScalarField& oldCellMask)
// {
	// labelList maskList(map.nOldCells());
	 // Info << "the mesh size is" << mesh_.nCells() << nl << endl;
	 // Info << "the maskList size is" << maskList.size() << nl << endl;
	 // Info << "the size of oldCellMask before equal" << oldCellMask.size() << nl << endl;
	 // forAll(oldCellMask.internalField(), cellI)
	   // {
		 // maskList[cellI] = oldCellMask[cellI];
	   // }         
	    // Info << "the size of oldCellMask" << oldCellMask.size() << nl << endl;
	 // Info << "distributed mesh 3083" << nl << endl;
	    // map.distributeCellData(maskList);
	// Info << "distributed mesh 3088" << nl << endl;
	    // oldCellMask.clear();
		 // Info << "distributed mesh 3087" << nl << endl;
	    // oldCellMask.resize(mesh_.nCells());
	 // Info << "distributed mesh 3089" << nl << endl;
	     // forAll(oldCellMask.internalField(), cellI)
	     // {
		  // oldCellMask[cellI] = maskList[cellI];
	     // }
	     // oldCellMask.correctBoundaryConditions();           
// }
