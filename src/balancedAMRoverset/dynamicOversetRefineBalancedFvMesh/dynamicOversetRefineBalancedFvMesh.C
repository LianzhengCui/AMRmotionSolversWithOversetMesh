...
#include "dynamicOversetRefineBalancedFvMesh.H"
...
namespace Foam
{
    defineTypeNameAndDebug(dynamicOversetRefineBalancedFvMesh, 0);
    addToRunTimeSelectionTable(dynamicFvMesh, dynamicOversetRefineBalancedFvMesh, IOobject);
    addToRunTimeSelectionTable(dynamicFvMesh, dynamicOversetRefineBalancedFvMesh, doInit);
}


// * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * * //

bool Foam::dynamicOversetRefineBalancedFvMesh::updateAddressing() const
{
    ...
    if (debug)
    {
        Pout<< "dynamicOversetRefineBalancedFvMesh::update() : extended addressing from"
...
Foam::scalar Foam::dynamicOversetRefineBalancedFvMesh::cellAverage
(
...
void Foam::dynamicOversetRefineBalancedFvMesh::writeAgglomeration
(
...
void Foam::dynamicOversetRefineBalancedFvMesh::protectHOLECells()
{
		dictionary DMDict
      (
          IOdictionary
        ...
        label interval =
        DMDict.getOrDefault<label>("protectInterval", 1);
		
		label protectInterval = (interval <= 0)?1:interval;
		
        if (time().timeIndex() % protectInterval == 0)
        {
			const cellCellStencilObject& overlap = Stencil::New(*this);
...
Foam::dynamicOversetRefineBalancedFvMesh::dynamicOversetRefineBalancedFvMesh
(
    const IOobject& io,
    const bool doInit
)
:
    dynamicMotionSolversRefineBalancedFvMesh(io, doInit)
{
    if (doInit)
    {
...
bool Foam::dynamicOversetRefineBalancedFvMesh::init(const bool doInit)
{
    if (doInit)
    {
        dynamicMotionSolversRefineBalancedFvMesh::init(doInit);
    }
...
const Foam::lduAddressing& Foam::dynamicOversetRefineBalancedFvMesh::lduAddr() const
{
    if (!active_)
    {
...
Foam::lduInterfacePtrsList Foam::dynamicOversetRefineBalancedFvMesh::interfaces() const
{
    if (!active_)
    {
...
Foam::dynamicOversetRefineBalancedFvMesh::primitiveLduAddr() const
{
    if (!lduPtr_)
    {
...
bool Foam::dynamicOversetRefineBalancedFvMesh::update()
{        
        // prevent the HOLE cells to be refined 
		// protectHOLECells();
		
		//call normal mesh update
		dynamicMotionSolversRefineBalancedFvMesh::update();
...
Foam::word Foam::dynamicOversetRefineBalancedFvMesh::baseName(const word& name)
{
...
bool Foam::dynamicOversetRefineBalancedFvMesh::interpolateFields()
{
...
bool Foam::dynamicOversetRefineBalancedFvMesh::writeObject
(
...
) const
{
    ...
    bool ok = dynamicMotionSolversRefineBalancedFvMesh::writeObject(streamOpt, valid);
