...
bool Foam::cellCellStencils::cellVolumeWeight::update()
{
	...
	{
			const labelIOList& indexMap = 
					      mesh_.lookupObject<labelIOList>("cellIndexMap");
						  
		forAll(allCellTypes, celli)
        {
		      			
		    bool cellTypeChange = 
			        (cellTypes_[indexMap[celli]] == HOLE);
							
			
           if (allCellTypes[celli] == CALCULATED && cellTypeChange)
            {
	...
	else
	{
        ...
        forAll(allCellTypes, celli)
        {
			bool cellTypeChange = (cellTypes_[celli] == HOLE);
    ...
    if (debug)
    {
        ...
        // dynamicOversetRefineBalancedFvMesh::correctBoundaryConditions
		dynamicOversetRefineBalancedFvMesh::correctBoundaryConditions
        <
            volScalarField,
